...
const _codeMap = new Map(COUNTRIES.map((c) => [c.code.toLowerCase(), c.name]))
const _nameSet = new Set(COUNTRIES.map((c) => c.name.toLowerCase()))
...
export function normalizeCountry(input: string | undefined): string | undefined {
  ...
  if (_nameSet.has(lower)) {
    return COUNTRIES.find((c) => c.name.toLowerCase() === lower)!.name
  }
  // ISO code match (2-char)
  const byCode = _codeMap.get(lower)
  if (byCode) return byCode
  // unrecognised — return as-is so the row isn't rejected
  return trimmed
}
