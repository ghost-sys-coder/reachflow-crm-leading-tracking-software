...
function CreateForm() {
  ...
  return <Card><CardHeader><CardTitle>Create a hosted lead form</CardTitle></CardHeader><CardContent>
    <form action={action} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2"><div><Label htmlFor="form-platform">Default outreach platform</Label><select id="form-platform" name="platform" defaultValue="other" disabled={pending} className="mt-1.5 h-9 w-full rounded-lg border bg-background px-3 text-sm">{["instagram","email","facebook","linkedin","x","call","other"].map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div><Label htmlFor="submit-label">Button label</Label><Input id="submit-label" name="submit_label" defaultValue="Send details" disabled={pending}/></div></div>
