...
export function SavedViewsBar({ initialViews, filters }: { initialViews: SavedView[]; filters: Filters }) {
  ...
  return <section className="flex min-w-0 max-w-full flex-col gap-3 rounded-xl border bg-card p-3 lg:flex-row lg:items-center"><div className="flex min-w-0 flex-1 flex-wrap gap-2">{views.map((view) => <span key={view.id} className="inline-flex max-w-full items-center rounded-md border"><Link className="min-w-0 truncate px-3 py-1.5 text-xs hover:bg-muted" href={hrefFor(view.filter_json)}>{view.name}{view.scope === "shared" ? " · Shared" : ""}</Link><button type="button" className="shrink-0 border-l p-1.5 text-muted-foreground hover:text-destructive" aria-label={`Delete ${view.name}`} onClick={() => remove(view.id)}><Trash2 className="size-3"/></button></span>)}{!views.length && <p className="py-1 text-xs text-muted-foreground">No saved views yet.</p>}</div><div className="flex min-w-0 flex-col gap-2 sm:flex-row"><Input className="h-8 min-w-0 sm:w-44" value={name} onChange={(event) => setName(event.target.value)} placeholder="View name"/><Button className="shrink-0" size="sm" variant="outline" disabled={pending || !name.trim()} onClick={save}><BookmarkPlus/>Save current view</Button></div></section>
}
